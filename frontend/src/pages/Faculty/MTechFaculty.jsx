...
const MTechFaculty = () => {
    ...
    useEffect(() => {
        fetch('/api/departments')
            ...
            .then(data => {
                if(data.data) {
                    // FILTER: Only show Engineering departments for M.Tech
                    const mTechCodes = ['CSE', 'CE', 'EE', 'ME', 'ECE', 'IT'];
                    const filtered = data.data.filter(dept => mTechCodes.includes(dept.code));
    ...
    if (loading) return <div className="content-wrapper"><p>Loading M.Tech faculty data...</p></div>;

    return (
        <div className="content-wrapper">
            <h2>M.Tech Departments & Faculty</h2>
            <p style={{marginBottom: '2rem', color: 'var(--text-secondary)'}}>
                Advanced postgraduate engineering research and faculty details.
            </p>
            ...
            {selectedDept && (
                <div style={modalOverlayStyle} onClick={closeModal}>
                    <div style={modalContentStyle} onClick={e => e.stopPropagation()}>
                        ...
                        <h2 style={{color: 'var(--accent-color)', borderBottom: '1px solid var(--accent-color)', paddingBottom: '10px'}}>
                            {selectedDept.name} (M.Tech)
                        </h2>
                        ...
                        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px', marginTop: '20px'}}>
                            ...
                            {/* BLOCK 3: RESEARCH & LABS */}
                            {selectedDept.roomNumbers && selectedDept.roomNumbers.length > 0 && (
                                <div style={infoBoxStyle}>
                                    <h4 style={{color: 'var(--accent-color)'}}>🔬 Research Labs</h4>
...
// --- STYLES (Identical to BTech Page for Consistency) ---
